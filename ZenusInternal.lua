...
do
	...
	function ZenusEnum.__index(t, k)
		return t._map[k] or ZenusEnum[k];
	end
	
	function ZenusEnum.new(items: {any}): {any}
		local map = {};
	...
	function ZenusEnum:GetEnumItems()
		return self._items;
	end
	
	--[[ Module ]]--
	
	framework.dependencies.ZenusEnum = {
		NavbarState = ZenusEnum.new({ "Hidden", "Partial", "Full" })
	};
end
...
do
	...
	local function saveUserSettings()
		if writefile then
			writefile("ZenusSettings.json", httpService:JSONEncode(tableUtils:DeepCopy(settingsCache)));
		end
	end
	...
	function userSettings:Initialize()
		if isfile and isfile("ZenusSettings.json") then
			local succ, res = pcall(httpService.JSONDecode, httpService, readfile("ZenusSettings.json"));
			if succ then
...
do
	...
	local function loadScriptCache()
		if isfile and isfile("ZenusScriptCache.json") then
			local s, r = pcall(httpService.JSONDecode, httpService, readfile("ZenusScriptCache.json"));
	...
	local function saveScriptCache()
		if writefile then
			...
			writefile("ZenusScriptCache.json", httpService:JSONEncode(cache));
		end
	end
	...
	framework.data.builtInScripts = {
		{
			...
			content = "loadstring(game:HttpGetAsync(\"https://raw.githubusercontent.com/Hirokuuu/Scripts/main/Script4\"))();"
		},
		{
			title = "Unnamed ESP",
...
do
	...
	local tabSystem = {
		...
		cache = {
			{
				title = "Tab",
				content = "-- Zenus Scripting Utility",
				index = 1
			}
		},
	...
	local function loadTabCache()
		if isfile and isfile("ZenusTabs.json") then
			local s, r = pcall(httpService.JSONDecode, httpService, readfile("ZenusTabs.json"));
			if s and type(r) == "table" then
				local accumulation = 0;
				local cache = {};
				local hasFoundDuplicateIndex = false;
	...
	function tabSystem:Save()
		if writefile then
			writefile("ZenusTabs.json", httpService:JSONEncode(tableUtils:DeepCopy(self.cache)));
		end
	end
...
do
	...
	local function checkWhitelist()
		if getgenv then
			return internalUtils:Request("https://api.Zenus.lol/v1/auth/authenticate", "POST") ~= true;
		end
		return false;
	end
	...
	local function changeTab(isMainTab: boolean)
		...
		ui.note.Text = isMainTab and "Please complete the whitelist to gain access to Zenus" or "Please enter your key to activate your Premium License";
	end
	...
	local function createUI(directory: Instance): ScreenGui
		ui = instanceUtils:Create("ScreenGui", {
			...
			textLabel({
				...
				Text = "Please complete the whitelist to gain access to Zenus",
				TextColor3 = Color3.fromRGB(159, 164, 186)
			}),
			instanceUtils:Create("Frame", {
				AnchorPoint = Vector2.new(1, 0),
				...
				textButton({
					...
					MouseButton1Click = function()
						if setclipboard then
							local data = internalUtils:Request("https://api.Zenus.lol/v1/auth/session", "POST");
							if data then
								setclipboard("https://mobile.Zenus.lol?token=" .. httpService:JSONDecode(data).token);
							end
						end
			...
			instanceUtils:Create("Frame", { 
				...
				textButton({
					...
					MouseButton1Click = function()
						...
						if #key > 0 then
							local res = internalUtils:Request("https://api.Zenus.lol/v1/auth/claim", "POST", {
...
do
	...
	local instanceUtils = framework.dependencies.utils.instance;
	local mathsUtils = framework.dependencies.utils.maths;
	local ZenusEnum = framework.dependencies.ZenusEnum;
			...
			if (input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch) and mathsUtils:IsWithin2DBounds(input.Position, dragBar.AbsolutePosition, dragBar.AbsolutePosition + dragBar.AbsoluteSize) then
				...
				local endedConn; endedConn = input.Changed:Connect(function(property)
					if input.UserInputState == Enum.UserInputState.End then
						...
						navbar:SetState(ZenusEnum.NavbarState[input.Position.X > 140 and "Full" or input.Position.X > 40 and "Partial" or "Hidden"]);
					end
				end);
			end
		end);
	...
	local function createNavbar(gui: ScreenGui): Instance
		...
		bar.indent:GetPropertyChangedSignal("Value"):Connect(function()
			...
			navbar.bar.main.ZenusIcon.Size = UDim2.new(0, 36 + percentage * 12, 0, 36 + percentage * 12);
	...
	function navbar:Initialize(directory: Instance)
		...
		self:SetState(ZenusEnum.NavbarState.Full, true);
	end
	...
	function navbar:SetState(navbarState: number, ignoreTimeouts: boolean?)
		...
		if navbarState == ZenusEnum.NavbarState.Partial then
			indent, state = 76, "partial";
		elseif navbarState == ZenusEnum.NavbarState.Full then
		...
		if state ~= "hidden" and not ignoreTimeouts then
			self.timeoutDelay = task.delay(5, function()
				if self.state == state then
					self:SetState(ZenusEnum.NavbarState.Hidden);
				end
			end);
	...
	function navbar:Select(button: TextButton)
		...
		self:SetState(ZenusEnum.NavbarState.Partial);
		instanceUtils:Tween(self.background, 0.2, {
			BackgroundTransparency = 0.1
		});
			...
			items = {
				...
				},
				{
					...
					callback = function(state)
						if state == false and isfile and isfile("ZenusTabs.json") then
							delfile("ZenusTabs.json");
						end
					end
				},
				{
